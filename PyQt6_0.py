import sys, os, threading, asyncio, shutil
import requests, torch, pygame, webbrowser
...
from PyQt5.QtWidgets import (
    ...
    QTextEdit, QDateEdit, QLineEdit, QHBoxLayout, QMessageBox,
    QComboBox, QTabWidget, QFileDialog, QProgressDialog
)
...
def gerar_relatorio_ia(descricao):
    ...
    try:
        prompt = (
            f"Gere um relatório detalhado em PORTUGUÊS-BR sobre: {descricao}. "
            f"Use títulos em MAIÚSCULAS e sem Markdown, ou seja, nao use "
            f"(*, #, =, asterisco, nem nada para deixar o texto em negrito)"
        )
...
def tocar_audio(texto, voz, botao):
    ...
    def _run():
        try:
            ...
            async def _gen():
                await edge_tts.Communicate(texto[:10000], voz).save(arquivo)
            asyncio.run(_gen())
...
def exibir_conteudo(url, titulo, expl):
    ...
    try:
        ...
        lbl_img.setPixmap(
            QPixmap(IMG_TEMP).scaled(lbl_img.size(), Qt.KeepAspectRatio, Qt.SmoothTransformation)
        )
...
def buscar_keyword():
    ...
    try:
        res = requests.get(
            f"https://images-api.nasa.gov/search?q={termo}&media_type=image"
        ).json()
        ...
        for i in itens:
            ...
            combo_res.setItemData(
                combo_res.count() - 1,
                {"url": link, "title": d.get('title'), "expl": d.get('description')},
                Qt.UserRole
            )
        combo_res.show()
        combo_res.showPopup()
...
def buscar_apod():
    ...
    try:
        res = requests.get(
            f"https://api.nasa.gov/planetary/apod?api_key={NASA_API_KEY}&date={data_str}"
        ).json()
        if 'url' in res:
...
def salvar_img():
    ...
    if not os.path.exists(IMG_TEMP):
        return
    path, _ = QFileDialog.getSaveFileName(None, "Salvar Imagem", "", "JPEG (*.jpg);;PNG (*.png)")
    if path:
        shutil.copy(IMG_TEMP, path)


def gerar_vid():

    if not data_store['en_expl']:
        return

    prog = QProgressDialog("Gerando vídeo com IA, aguarde...", "Cancelar", 0, 0)
    prog.show()
    ...
    def _worker():
        try:
            ...
            adapter = MotionAdapter.from_pretrained(
                "guoyww/animatediff-motion-adapter-v1-5-2", torch_dtype=typ
            )
            pipe = AnimateDiffPipeline.from_pretrained(
                "runwayml/stable-diffusion-v1-5", motion_adapter=adapter, torch_dtype=typ
            )
            ...
            out = pipe(
                prompt=f"Space, {data_store['en_expl'][:100]}",
                num_frames=16,
...
top_bar    = QHBoxLayout()
edit_busca = QLineEdit(); edit_busca.setPlaceholderText("Pesquisar na NASA Images API...")
...
for w in [edit_busca, btn_busca, edit_data, btn_apod]:
    top_bar.addWidget(w)
...
def add_tab(nome, cor, voz):

    w = QWidget(); layout = QVBoxLayout(w)
    txt = QTextEdit(); txt.setReadOnly(True)
    btn = QPushButton(f"OUVIR {nome}")
    btn.setStyleSheet(f"background: {cor}; color: white; font-weight: bold; padding: 8px;")
    btn.clicked.connect(lambda: tocar_audio(txt.toPlainText(), voz, btn))
...
for b in [btn_stop, btn_save, btn_vid]:
    bot_bar.addWidget(b)
...
combo_res.currentIndexChanged.connect(
    lambda i: exibir_conteudo(
        combo_res.itemData(i)['url'],
        combo_res.itemData(i)['title'],
        combo_res.itemData(i)['expl']
    ) if i > 0 else None
)
